...
func problemtestAddressV2WithGroupKey(t *harnessTest) {
	// We begin by minting a new asset group with a group key.
	firstTrancheReq := CopyRequest(issuableAssets[0])
	...
	totalAmount := firstAsset.Amount + secondAsset.Amount + thirdAsset.Amount
	t.Logf("Minted %d units for group %x", totalAmount, groupKey)
	...
	// send the first tranche from alice to bob
	...
	// send the second tranche from alice to bob
	...
	// send the third tranche from alice to bob
	...
	// note: tapd seems to coin select in a different order than issuance every time so this doesn't work
	// AssertAssetOutboundTransferWithOutputs(
	// 	t.t, t.lndHarness.Miner().Client, bobTapd,
	// 	sendResp.Transfer, [][]byte{firstAssetID, secondAssetID, thirdAssetID},
	// 	[]uint64{0,firstAsset.Amount, 0, secondAsset.Amount, 0, thirdAsset.Amount}, bobcurrentTransferIdx,
	// 	bobnumTransfers, 6, true,
	// )

	// hack to make the compiler not complain because the above lines are commented out
	_= bobcurrentTransferIdx
	...
	for i := 0; i < 4; i++{		// repeate this section a few times

		// now send back from alice to bob again, but this time all at once instead of individually
		...
		// return from bob back to alice
		// this time, we'll again send back all 3 tranches at once.
	...
	// the above loop can be run as much as we want, but as soon as we try to
	// send the assets invidually a second time, things go wrong

	// send the first tranche back to bob
	...
	// send the second tranche back to bob
	...
	// fails, (bob never received the transfer)
	AssertAddrEventByStatus(t.t, bobTapd, statusCompleted, numTransfers)


	// send the third tranche back to bob (can't even get here because
	// of the above
	// `AssertAddrEventByStatus(t.t, bobTapd, statusCompleted, numTransfers)`)
	// failed, but if you comment this line out it will still fail because of
	// https://github.com/lightninglabs/taproot-assets/issues/1738
