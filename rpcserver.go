	...
	case pReq.PaymentRequest != "":
		...
		for _, quote := range acquiredQuotes {
			err := notifyUserForPaymentQuotes(
				stream, quote, payHash, peerPubKey,
			)
			if err != nil {
				return err
			}
...
func notifyUserForPaymentQuotes(
	...
	quote *rfqrpc.PeerAcceptedSellQuote, payHash string,
	...
	if err != nil {
		rpcsLog.Errorf("error unmarshalling asset "+
			"rate for payment %v: %w", payHash, err)
	}

	rpcsLog.Infof("Payment %v got quote for %v asset "+
		"units at %v asset/BTC from peer %x with "+
		"SCID %d", payHash, quote.AssetAmount,
		assetRate, peerPubKey, quote.Scid)
